...
if [ $# -eq 2 ]
then
    echo "Looking for something at coords: $1 $2"
...
res=$(mysql -u$user -p$password $db -N -e "SELECT sdbid FROM xids WHERE xid='$sdbid';")
if [[ $res = $sdbid ]]
then
...
then
    ...
    $stilts tjoin nin=2 in1=$ft ifmt1=ascii icmd1='keepcols sdbid' in2=$ft2 ifmt2=votable ocmd='random' omode=tosql protocol=mysql db=$db user=$user password=$password dbtable=simbad write=$mode
fi

# add sdbid as an xid, and given id. latter is a potential issue if given id not
# unique. sdbid will be included in matches below. using ascii table here works better
# since the header isn't recognised when in csv
if [ "$id" != "" ] || 0 # SKIP FOR NOW
then
    echo $sdbid $id >> $ft
fi
$stilts tpipe in=$ft ifmt=ascii cmd='random' omode=tosql protocol=mysql db=$db user=$user password=$password dbtable=xids write=append
...
then
    ...
    $stilts sqlclient db='jdbc:mysql://localhost/photometry' user=$user password=$password sql="SELECT * from iras_fsc where _raj2000 between $ra-5.0 and $ra+5.0 and _dej2000 between $de-5.0 and $de+5.0" ofmt=votable > $ft
    $stilts tmatch2 in1=$fp ifmt1=votable in2=$ft ifmt2=votable ocmd='keepcols "sdbid IRAS_ID"' matcher=skyellipse values1='ra_ep1983p5 de_ep1983p5 1.0 1.0 0.0' values2='_RAJ2000 _DEJ2000 Major Minor PosAng' params='20' omode=tosql protocol=mysql db=$db user=$user password=$password dbtable=xids find=best write=append
else
...
then
    ...
    $stilts sqlclient db='jdbc:mysql://localhost/photometry' user=$user password=$password sql="SELECT * from iras_psc where _raj2000 between $ra-5.0 and $ra+5.0 and _dej2000 between $de-5.0 and $de+5.0" ofmt=votable > $ft
    $stilts tmatch2 in1=$fp ifmt1=votable in2=$ft ifmt2=votable ocmd='keepcols "sdbid IRAS_ID"' matcher=skyellipse values1='ra_ep1983p5 de_ep1983p5 1.0 1.0 0.0' values2='_RAJ2000 _DEJ2000 Major Minor PosAng' params='20' omode=tosql protocol=mysql db=$db user=$user password=$password dbtable=xids find=best write=append
else
...
coty=$(mysql -u$user -p$password $db -N -e "SELECT CONCAT(ra_ep1991p25,',',de_ep1991p25) FROM sdb_pm WHERE sdbid='$sdbid';")
echo $coty
...
$stilts tjoin nin=2 in1=$fp ifmt1=votable icmd1='keepcols sdbid' in2=$ft ifmt2=votable ocmd='random' omode=tosql protocol=mysql db=$db user=$user password=$password dbtable=tyc2 write=$mode

# 2MASS, mean epoch of 1999.3, midway through survey 2006AJ....131.1163S
cotm=$(mysql -u$user -p$password $db -N -e "SELECT CONCAT(ra_ep1999p3,',',de_ep1999p3) FROM sdb_pm WHERE sdbid='$sdbid';")
vizquery -site=$site -mime=votable -source=2mass -c.rs=$rad -sort=_r -out.max=1 -out.add=_r -c="$cotm" > $ft
$stilts tjoin nin=2 in1=$fp ifmt1=votable icmd1='keepcols sdbid' in2=$ft ifmt2=votable ocmd='random' omode=tosql protocol=mysql db=$db user=$user password=$password dbtable=2mass write=$mode

# ALLWISE, assume 2010.3, midway through cryo lifetime
cowise=$(mysql -u$user -p$password $db -N -e "SELECT CONCAT(ra_ep2010p3,',',de_ep2010p3) FROM sdb_pm WHERE sdbid='$sdbid';")
vizquery -site=$site -mime=votable -source=II/328/allwise -out.add=_r -c.rs=$rad -sort=_r -out.max=1 -c="$cowise" > $ft
$stilts tjoin nin=2 in1=$fp ifmt1=votable icmd1='keepcols sdbid' in2=$ft ifmt2=votable ocmd='random' omode=tosql protocol=mysql db=$db user=$user password=$password dbtable=allwise write=$mode

# AKARI IRC, assume 2007.0, midway through survey
coirc=$(mysql -u$user -p$password $db -N -e "SELECT CONCAT(ra_ep2007p0,',',de_ep2007p0) FROM sdb_pm WHERE sdbid='$sdbid';")
vizquery -site=$site -mime=votable -source=II/297 -out.add=_r -c.rs=$rad -sort=_r -out.max=1 -c="$coirc" | grep -v "^#" > $ft
$stilts tjoin nin=2 in1=$fp ifmt1=votable icmd1='keepcols sdbid' in2=$ft ifmt2=votable ocmd='random' omode=tosql protocol=mysql db=$db user=$user password=$password dbtable=akari_irc write=$mode
...
$stilts tjoin nin=2 in1=$fp ifmt1=votable icmd1='keepcols sdbid' in2=$ft ifmt2=votable ocmd='random' omode=tosql protocol=mysql db=$db user=$user password=$password dbtable=seip write=$mode
