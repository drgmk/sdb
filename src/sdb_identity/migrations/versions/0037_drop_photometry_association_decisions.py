"""Drop photometry association decisions (set-scope retired).

Photometry review is now expressed through three distinct concerns: ownership
(``photometry assign``), fit eligibility (``photometry include``/``exclude``),
and catalog-candidate triage. The overloaded ``set-scope`` command and its
``photometry_association_decisions`` table are removed; its only functional
effect (``reject`` -> excluded) is fully covered by exclude overrides.
"""
...
revision = "0037_drop_photometry_association_decisions"
down_revision = "0036_canonical_catalog_detections"
branch_labels = None
depends_on = None
