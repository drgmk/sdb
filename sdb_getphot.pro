...
pro sdb_getphot,id1,location=loc,project=project,VERBOSE=verb
  ...
  if KEYWORD_SET(project) then begin
     mysqlquery,lun,'SELECT ppmxl FROM projects WHERE project="'+project+'";',id,V=KEYWORD_SET(verb),F='a',N=n
  ...
  ;; get PPMXL id and xids
  mysqlquery,lun,'SELECT DISTINCT ppmxl FROM xids WHERE xid="'+id1+'";',ppmxl,VERBOSE=KEYWORD_SET(verb),format='a'
  mysqlquery,lun,'SELECT DISTINCT xid FROM xids WHERE ppmxl='+ppmxl+';',xids,VERBOSE=KEYWORD_SET(verb)
  print,'ID: ',id1,ppmxl

  ;; make cross id table to match on
  mysqlcmd,lun,"DROP TABLE IF EXISTS TheIDs;"
  mysqlcmd,lun,'CREATE TEMPORARY TABLE TheIDs SELECT * from xids WHERE ppmxl = '+ppmxl+';',DEBUG=KEYWORD_SET(verb)
  ...
  if n eq 0 then begin
     print,'No photometry for ',id1,ppmxl
     free_lun,lun
     return
  endif
  ...
  mysqlquery,lun,'SELECT sp_type,sp_bibcode,plx_value,plx_err,plx_bibcode from simbad WHERE ppmxl='+ppmxl+';',spty,spty_ref,plx,e_plx,plx_ref,VERBOSE=KEYWORD_SET(verb),format='a,a,a,a,a'
  free_lun,lun
  ...
  stinf = {ntries:0,name:ppmxl,xids:xids,spty:spty,spty_ref:spty_ref,plx:plx,eplx:e_plx,plx_ref:plx_ref,dist:FLOAT(1e3/plx),teff:FLOAT(teff),tefferr:tefferr,logg:FLOAT(logg),mh:FLOAT(mh),av:0.}
        
  ;; save file (using given name)
  fn = loc+ppmxl+'/'+ppmxl+'.xdr'
  if ~ FILE_TEST(loc+ppmxl,/DIRECTORY) then begin ; if no dir then create
     FILE_MKDIR,loc+ppmxl
  endif else begin
